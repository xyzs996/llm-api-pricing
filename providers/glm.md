...
Every GLM model in the catalog that has been ranked in an agent category, priced three ways — list, cache read, and what the two come to at the token mix an agent actually sends. Recomputed from the source catalog on **2026-08-29**.
...
6 GLM models that have been ranked in an agent category of the Design Arena, read from [OpenRouter](https://openrouter.ai/models)'s public catalog on **2026-08-29**. Three prices per row: what the row lists, what a cache read costs, and what the two come to at the token mix a coding agent actually sends.

**GLM does not have one cache-read rate — it has 2.** Across 6 rows the discount runs from 18.6% to 20.0% of that row's own input price (18.6%, 20%). So a cheaper list price here can still be the dearer call once an agent starts caching, and no single discount figure describes this vendor. Repriced at a coding agent's mix, GLM's list input price overstates what an agent pays by a median **4.2×** (range 4.1×–4.4×).

| $ / 1M at agent mix | $ in / 1M | $ cache read / 1M | $ out / 1M | Model | Context | Long-context step | Best agents rank |
| --- | --- | --- | --- | --- | --- | --- | --- |
| **$0.0979** | $0.40 | $0.08 | $1.75 | [GLM 4.7](https://openrouter.ai/z-ai/glm-4.7) | 204K | — | #28 androidnative |
...
| **$0.2706** | $1.19 | $0.221 | $3.74 | [GLM 5.2](https://openrouter.ai/z-ai/glm-5.2) | 1M | — | #9 agenticgamedev |
| **$0.2866** | $1.26 | $0.234 | $3.96 | [GLM 5.1](https://openrouter.ai/z-ai/glm-5.1) | 204K | — | #3 agenticslides |
| **$0.29** | $1.20 | $0.24 | $4.00 | [GLM 5V Turbo](https://openrouter.ai/z-ai/glm-5v-turbo) | 202K | — | #5 androidnative |
