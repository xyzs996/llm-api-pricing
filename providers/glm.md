...
Every GLM model in the catalog that has been ranked in an agent category, priced three ways — list, cache read, and what the two come to at the token mix an agent actually sends. Recomputed from the source catalog on **2026-09-23**.
...
8 GLM models that have been ranked in an agent category of the Design Arena, read from [OpenRouter](https://openrouter.ai/models)'s public catalog on **2026-09-23**. Three prices per row: what the row lists, what a cache read costs, and what the two come to at the token mix a coding agent actually sends.

**GLM does not have one cache-read rate — it has 3.** Across 8 rows the discount runs from 16.7% to 20.0% of that row's own input price (16.7%, 18.6%, 20%). So a cheaper list price here can still be the dearer call once an agent starts caching, and no single discount figure describes this vendor. Repriced at a coding agent's mix, GLM's list input price overstates what an agent pays by a median **4.4×** (range 4.1×–4.8×).
...
| **$0.0991** | $0.43 | $0.08 | $1.75 | [GLM 4.6](https://openrouter.ai/z-ai/glm-4.6) | 204K | — | #13 godotgamedev |
| **$0.1277** | $0.5614 | $0.1043 | $1.7644 | [GLM 5.3](https://openrouter.ai/z-ai/glm-5.3) | 1.3M | — | #7 python-pptxslides |
...
| **$0.151** | $0.72 | $0.12 | $2.40 | [GLM 5.3](https://openrouter.ai/z-ai/glm-5.3:batch) `batch` | 1M | — | #7 python-pptxslides |
