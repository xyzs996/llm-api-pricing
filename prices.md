...
**One row here was checked against its vendor's own page, and it is wrong in every column and nearly right in the one that matters.** `deepseek/deepseek-v4-pro` is quoted from the catalog at $0.4138 in and $0.0345 cache-read. Calling `deepseek-v4-pro` direct, per [DeepSeek's pricing page](https://api-docs.deepseek.com/quick_start/pricing/) read 2026-08-23, the input price is **1.6× higher** off-peak and **3.2× higher** at peak — but the cache-read price is **1.6× *lower*** at the vendor off-peak. The two errors point in opposite directions, and because 95.6% of an agent's tokens are cache reads they very nearly cancel: $0.0536 per million at the vendor off-peak against $0.0522 for the catalog row, **103% of each other**. At peak they do not cancel — $0.1073, or 2.1× the catalog row.
